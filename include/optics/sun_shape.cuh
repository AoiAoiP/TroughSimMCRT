...
namespace Optics{
    __device__ inline float SampleThetaFromcdf(float u,const SunConfig& config){
        // binary search over CDF lookup table
        int left = 0;
    ...
    __device__ inline float3 SampleSunshape(float u1,float u2,const SunConfig& config){
        ...
        // build local orthonormal basis (U, V, W) where W = sun direction
        float3 a;
        if (fabsf(w.x) > 0.9f) {
        ...
        // map local direction to world-space 3D ray (equivalent to local2world)
